...
#create dataset
df1<-data.frame(state=c('Iowa','Vermont','Hawaii','Texas',"Alaska"),var1=c('american goldfinch',"hermit thrush",'nene','northern mockingbird',"willow ptarmigan"),snow=c(1,1,0,0,1),number.of.cats=sample(3:10,5,replace=T))
#explore dataset
library(ggplot2)
ggplot(df1,aes(state,number.of.cats))+geom_point()
#create new column
df1 $ max=10
#analyze dataset
library(lme4)
